...
<button><a style = "text-decoration:none; color:#000;" href = "view_covid_record.php">Back</a></button>
<?php
	$_SESSION['date'] = date("Y-m-d", strtotime("+ 8 HOURS"));
	$conn = new mysqli("localhost", "root", "", "hcpms") or die(mysqli_error());
	$q = $conn->query("SELECT * FROM `covid` WHERE `date` = '$_SESSION[date]'") or die(mysqli_error());
	$f = $q->fetch_array();
?>
<form style = "float:right;" method = "POST" action = "covid_print_sort.php">
...
<br />
	<div id="print">
		<div style = "margin:10px;">	<br />		
			...
			<center>Province of Negros Occicovid</center>
			...
			<center>
				...
				<?php
					$query = $conn->query("SELECT * FROM `covid` NATURAL JOIN `itr` WHERE `date` = '$_SESSION[date]' LIMIT 30") or die(mysqli_error());
					for($a = 1; $a <= 30; $a++){
