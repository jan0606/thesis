...
<body>
	...
	<div id = "content">
		...
		<?php
			$conn = new mysqli("localhost", "root", "", "hcpms") or die(mysqli_error());
			$q1 = $conn->query("SELECT * FROM `covid` NATURAL JOIN `itr` WHERE `itr_no` = '$_GET[itr_no]'") or die(mysqli_error());
			$f1 = $q1->fetch_array();
		?>
		<div class = "alert alert-info">covid Record / <?php echo $f1['firstname']." ".substr($f1['middlename'], 0,1).$f1['lastname']?><a class = "btn btn-success" style = "float:right; margin-top:-7px;" href = "covid.php"><span class = "glyphicon glyphicon-hand-right"></span> Back</a></div>
		<table id = "table" cellspacing = "0" class = "display">
			...
			<?php 
				$conn = new mysqli("localhost", "root", "", "hcpms") or die(mysqli_error());
				$q = $conn->query("SELECT * FROM `covid` NATURAL JOIN `itr` WHERE `itr_no` = '$_GET[itr_no]' ORDER BY `date` DESC") or die(mysqli_error());
			...
			?>
				<tr>
					...
					<td><center><a class = "btn btn-info" href = "covid_form.php?itr_no=<?php echo $f['itr_no']?>&covid_no=<?php echo $f['covid_no']?>"><span class = "glyphicon glyphicon-search"></span> View Detail</a><center></td>
				</tr>
			<?php
				}
			?>	
